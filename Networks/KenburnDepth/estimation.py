...
class Semantics(torch.nn.Module):
    ...
    def forward(self, tenInput):
        ...
        tenPreprocessed = tenPreprocessed - torch.tensor(data=[0.485, 0.456, 0.406], dtype=tenPreprocessed.dtype,
...
# netDisparity.load_state_dict({strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in
#                                     torch.load("/home/godeta/Téléchargements/network-disparity.pytorch").items()})
#
#
#
...
#
#     tenImage = torch.nn.functional.interpolate(input=tensor(tenImage), size=(intHeight, intWidth), mode='bilinear',
#                                                align_corners=False)
#
