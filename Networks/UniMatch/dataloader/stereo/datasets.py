...
class StereoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if len(sample['left'].shape) == 2:
        #     temp = np.zeros(sample['left'].shape[0], sample['left'].shape[1], 3)
        #     temp[:, :, 0] = sample['left']
        #     temp[:, :, 1] = sample['left']
        #     temp[:, :, 2] = sample['left']
        #     sample['left'] = temp
        sample['right'] = read_img(sample_path['right'])
        # if len(sample['right'].shape) == 2:
        #     temp = np.zeros(sample['right'].shape[0], sample['right'].shape[1], 3)
        #     temp[:, :, 0] = sample['right']
        #     temp[:, :, 1] = sample['right']
        #     temp[:, :, 2] = sample['right']
        #     sample['right'] = temp
        ...
        # sample['left'] = cv2.resize(sample['left'], [sample['right'].shape[1], sample['right'].shape[0]], interpolation=cv2.INTER_LINEAR)
        # if True:
        #     print("The shapes are : ", sample['left'].shape, sample['right'].shape, sample['disp'].shape)
        #     time.sleep(1)
...
class Hybrid(StereoDataset):
    def __init__(self,
                 data_dir='/home/godeta/PycharmProjects/LYNRED/Video_frame/Day',
                 transform=None,
                 ):
        super(Hybrid, self).__init__(transform=transform, is_hybrid=True)

        # merge train and test
        left_files = sorted(glob(data_dir + '/hybrid/left/*.png'))
        right_files = sorted(glob(data_dir + '/hybrid/right/*.png'))
        disp_files = sorted(glob('/home/godeta/PycharmProjects/LYNRED/Stereo_matching/NeuralNetwork/UniMatch/dataset/disparity/*.png'))
