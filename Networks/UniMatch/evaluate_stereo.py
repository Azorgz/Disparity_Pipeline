...
from utils.disparity_tools import reconstruction_from_disparity
...
def inference_stereo(model,
    ...
    for i in range(num_samples):
        ...
        left_im = torch.from_numpy(left) / 255.
        right_im = torch.from_numpy(right) / 255.
        left_im = left_im.to(device).unsqueeze(0)
        right_im = right_im.to(device).unsqueeze(0)
        ...
        save_name = os.path.join(output_path + '/disp', os.path.basename(left_name)[:-4] + '_disp.png')
        save_name_image_reg = os.path.join(output_path + '/image_reg', os.path.basename(left_name)[:-4] + '_reg.png')
        save_name_image_reg_2 = os.path.join(output_path + '/image_reg', os.path.basename(left_name)[:-4] + '_reg_2.png')
        ## Timer for reconstruction
        t = time.time()
        if pred_right_disp:
            pred_disp = hflip(pred_disp)
        _, h, w = pred_disp.shape
        grid = kornia.utils.create_meshgrid(h, w, device=0).to(left.dtype)
        grid[:, :, :, 0] += pred_disp/pred_disp.shape[2]*2
        pred_disp_reg = grid
        # breakpoint()

        if pred_right_disp:
            new_image = torch.nn.functional.grid_sample(right_im.permute(0, 3, 1, 2), pred_disp_reg,
                                                        padding_mode='border',
                                                        align_corners=True)
        else:
            new_image = torch.nn.functional.grid_sample(left_im.permute(0, 3, 1, 2), pred_disp_reg,
                                                        padding_mode='border',
                                                        align_corners=True)
        print(f'GPU 1 {time.time() - t} sec')
        disp = pred_disp[0].cpu().numpy()
        print(f'To CPU {time.time() - t} sec')
        new_image = np.uint8(new_image[0].permute(1, 2, 0).cpu().numpy()*255)
        new_image = cv2.cvtColor(new_image, cv2.COLOR_RGB2BGR)
        print(f'Reconstruction using GPU took {time.time() - t} sec')
        t = time.time()
        if pred_right_disp:
            r = np.array(Image.open(right_name).convert('RGB')).astype(np.float32)
            new_image_2 = \
                reconstruction_from_disparity(r, r, disp,
                                          min_disp=0, max_disp=0, closing_bool=False, verbose=False,
                                          median=False, inpainting=False, copycat=False, orientation=0)
        else:
            l = np.array(Image.open(left_name).convert('RGB')).astype(np.float32)
            new_image_2 = \
                reconstruction_from_disparity(l, l, disp,
                                              min_disp=0, max_disp=0, closing_bool=False, verbose=False,
                                              median=False, inpainting=False, copycat=False, orientation=0)
        new_image_2 = cv2.cvtColor(np.uint8(new_image_2), cv2.COLOR_RGB2BGR)
        print(f'Reconstruction using Numpy and CPU took {time.time() - t} sec')
        # breakpoint()
        ...
        cv2.imwrite(save_name, disp)
        cv2.imwrite(save_name_image_reg, new_image)
        cv2.imwrite(save_name_image_reg_2, new_image_2)
        if pred_bidir_disp:
