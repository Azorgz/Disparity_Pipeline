...
def main(args):
    ...
    while total_steps < args.num_steps:
        model.train()
        print(f'{total_steps = }')
        print(f"{torch.cuda.memory_allocated(0) / 1024 ** 3} Go allocated")
        ...
        for i, sample in tqdm(enumerate(train_loader), total=args.num_steps, desc="Nombre d'itérations : "):
            ...
            # loss weights
            loss_weights = [0.9 ** (len(pred_disps) - 1 - power) for power in range(len(pred_disps))]
