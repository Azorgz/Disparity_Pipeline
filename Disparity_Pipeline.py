...
from module.PointCloudTransformer import PointCloudTransformer
# module
...
from module.Reconstruction import Reconstruction
...
class Pipe:
    ...
    def __init__(self, config: ConfigPipe):
        ...
        self._init_network_()
        self._init_alignment_()
        self.postprocessing = None
        self._init_preprocessing_()
    ...
    @torch.no_grad()
    def run(self):
        to_numpy_normalize = ToNumpy(normalize=True)
        to_numpy = ToNumpy(normalize=False, permute=False)
        for idx, sample in tqdm(enumerate(self.dataloader),
                                total=len(self.dataloader),
                                desc="Nombre d'itérations : "):

            # Alignment of the input images according the needs (Only left-right for 2IR setup) #######
            new_sample = self.alignment(sample, alignment_step=0)
            ...
            pred_disp = self.disparity_network(sample_preprocess['left'], sample_preprocess['right'])
    ...
    def _init_dataloader_(self):
        self.dataloader = StereoDataLoader(self.config)
        self.modules.append(self.dataloader)

    @torch.no_grad()
    def _init_alignment_(self):
        self.alignment = Alignment(self.dataloader, self.config, self.disparity_network)
        if not self.config['dataset']['alignment_isDone']:
            self.modules.append(self.alignment)

    @torch.no_grad()
    def _init_preprocessing_(self):
        self.preprocessing = Preprocessing(self.config)
        self.modules.append(self.preprocessing)

    @torch.no_grad()
    def _init_network_(self):
        self.disparity_network = SuperNetwork(config)
        self.network_name = self.disparity_network.name
        self.modules.append(self.disparity_network)
