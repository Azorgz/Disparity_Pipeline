...
from modulev2.BaseModule import BaseModule
from modulev2.Preprocessing import Preprocessing
...
class SuperNetwork(BaseModule):
    ...
    def __call__(self, sample, *args, depth=False, **kwargs):
        if not depth:
            ...
            if self.pred_bidir:
                left = DepthTensor(res[0])
                left.im_name = im_left.im_name
                right = DepthTensor(res[1])
            ...
            img_ref, img_tgt = sample['left'], sample['right']
            intrinsics, pose = args

            if self.name_depth == "unimatch":
                res = self.model_depth(img_ref, img_tgt,
            ...
            if self.pred_bidir:
                left = DepthTensor(res[0])
                left.im_name = sample['left'].im_name
                right = DepthTensor(res[1])
                right.im_name = sample['right'].im_name
                res = {'left': left, 'right': right}
            elif self.pred_right:
                right = DepthTensor(res[1])
                right.im_name = sample['right'].im_name
