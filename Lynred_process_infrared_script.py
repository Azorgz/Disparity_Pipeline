...
# for i in range(4):
#     path1 = f"/media/godeta/T5 EVO/Datasets/Lynred/sequence_{1 + i}/infrared_1_corrected/"
#     new_path1 = f"/media/godeta/T5 EVO/Datasets/Lynred/sequence_{1 + i}/infrared_1/"
#     path2 = f"/media/godeta/T5 EVO/Datasets/Lynred/sequence_{1 + i}/infrared_2_corrected/"
#     new_path2 = f"/media/godeta/T5 EVO/Datasets/Lynred/sequence_{1 + i}/infrared_2/"
#     os.makedirs(new_path1, exist_ok=True)
#     os.makedirs(new_path2, exist_ok=True)
#
#     histo = None
#     base_name = os.listdir(path1)[0].split('_')[0] + '_' + os.listdir(path1)[0].split('_')[1] + '_'
#     first_idx = np.min([int(x.replace(base_name, '').replace('.png', '')) for x in os.listdir(path1)])
#     for idx in tqdm(range(len(os.listdir(path1)))):
#         im = base_name + f'{first_idx + idx}.png'
#         ir = ImageTensor(path1 + im)
#         if histo is None:
#             histo = ir.hist()
#         else:
...
#         ir.name = f"{im.split('_')[0]}_{im.split('_')[1]}_{name_generator(idx, max_number=len(os.listdir(path1)))}"
#         ir.save(new_path1)
#
#     histo = None
#     base_name = os.listdir(path2)[0].split('_')[0] + '_' + os.listdir(path2)[0].split('_')[1] + '_'
#     first_idx = np.min([int(x.replace(base_name, '').replace('.png', '')) for x in os.listdir(path2)])
#     for idx in tqdm(range(len(os.listdir(path2)))):
#         im = base_name + f'{first_idx + idx}.png'
#         ir = ImageTensor(path2 + im)
#         if histo is None:
#             histo = ir.hist()
#         else:
#             histo = histo.__add__(ir.hist())
...
#         ir = ImageTensor(path2 + im)
#         ir = ir.histo_equalization(mini=mini, maxi=maxi)
#         ir.name = f"{im.split('_')[0]}_{im.split('_')[1]}_{name_generator(idx, max_number=len(os.listdir(path2)))}"
#         ir.save(new_path2)
